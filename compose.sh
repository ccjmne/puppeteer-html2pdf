...
docker build -t ${img}:${v} -t 424880512736.dkr.ecr.eu-west-1.amazonaws.com/${img}:${v} .
ok "Build complete"

nf "Push to our private container registry..."
docker push 424880512736.dkr.ecr.eu-west-1.amazonaws.com/${img}:${v}
ok "Push complete"

nf "\e[2mTesting:\e[0m"
nf "   docker run -it --rm -p=\e[1;34m<port>\e[0m:3000 ${img}:\e[1;34m${v}"
nf "\e[0m - \e[2mkill w/: \e[0mCtrl^C"
nf "\e[2mProduction:\e[0m"
nf "   docker run --detach --shm-size 1G --sysctl net.ipv6.conf.all.disable_ipv6=1 \\
                        --name ${img} -p=\e[1;34m<port>\e[0m:3000 ${img}:\e[1;34m${v}"
